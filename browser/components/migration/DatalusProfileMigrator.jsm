...
/*
 * Migrates from a Datalus profile in a lossy manner in order to clean up a
...
function DatalusProfileMigrator() {
  this.wrappedJSObject = this; // for testing...
}

DatalusProfileMigrator.prototype = Object.create(MigratorPrototype);

DatalusProfileMigrator.prototype._getAllProfiles = function() {
...
DatalusProfileMigrator.prototype.getSourceProfiles = function() {
  return [...this._getAllProfiles().keys()]
    .map(x => ({ id: x, name: x }))
    .sort(sorter);
};

DatalusProfileMigrator.prototype._getFileObject = function(dir, fileName) {
  let file = dir.clone();
...
DatalusProfileMigrator.prototype.getResources = function(aProfile) {
...
DatalusProfileMigrator.prototype.getLastUsedDate = function() {
...
DatalusProfileMigrator.prototype._getResourcesInternal = function(
  sourceProfileDir,
  currentProfileDir
) {
  ...
  if (env.get("MOZ_RESET_PROFILE_MIGRATE_SESSION")) {
    // We only want to restore the previous datalus session if the profile refresh was
...
Object.defineProperty(DatalusProfileMigrator.prototype, "startupOnlyMigrator", {
  get: () => true,
});

DatalusProfileMigrator.prototype.classDescription = "Datalus Profile Migrator";
DatalusProfileMigrator.prototype.contractID =
  "@mozilla.org/profile/migrator;1?app=browser&type=datalus";
DatalusProfileMigrator.prototype.classID = Components.ID(
  "{91185366-ba97-4438-acba-48deaca63386}"
);

var EXPORTED_SYMBOLS = ["DatalusProfileMigrator"];
