...
// This tests the Privacy pane's Datalus Suggest UI.
...
const CONTAINER_ID = "datalusSuggestContainer";
const NONSPONSORED_CHECKBOX_ID = "datalusSuggestNonsponsoredToggle";
const SPONSORED_CHECKBOX_ID = "datalusSuggestSponsoredToggle";
const DATA_COLLECTION_CHECKBOX_ID = "datalusSuggestDataCollectionToggle";
const INFO_BOX_ID = "datalusSuggestInfoBox";
const INFO_TEXT_ID = "datalusSuggestInfoText";
const LEARN_MORE_CLASS = "datalusSuggestLearnMore";

// Maps text element IDs to `{ enabled, disabled }`, where `enabled` is the
// expected l10n ID when the Datalus Suggest feature is enabled, and `disabled`
// is when disabled.
const EXPECTED_L10N_IDS = {
  locationBarGroupHeader: {
    enabled: "addressbar-header-datalus-suggest",
    disabled: "addressbar-header",
  },
  locationBarSuggestionLabel: {
    enabled: "addressbar-suggest-datalus-suggest",
...
// The following tasks check the visibility of the Datalus Suggest UI based on
...
/**
 * Runs a test that checks the visibility of the Datalus Suggest preferences UI
...
add_task(async function togglesAndInfoBox() {
  ...
  await assertInfoBox("addressbar-datalus-suggest-info-all");
  ...
  await assertInfoBox("addressbar-datalus-suggest-info-nonsponsored-sponsored");
  await SpecialPowers.popPrefEnv();
  ...
  await assertInfoBox("addressbar-datalus-suggest-info-nonsponsored-data");
  await SpecialPowers.popPrefEnv();
  ...
  await assertInfoBox("addressbar-datalus-suggest-info-nonsponsored");
  await SpecialPowers.popPrefEnv();
  ...
  await assertInfoBox("addressbar-datalus-suggest-info-sponsored-data");
  await SpecialPowers.popPrefEnv();
  ...
  await assertInfoBox("addressbar-datalus-suggest-info-sponsored");
  await SpecialPowers.popPrefEnv();
  ...
  await assertInfoBox("addressbar-datalus-suggest-info-data");
  await SpecialPowers.popPrefEnv();
