...
This document provides a high level, nontechnical overview of Datalus's address
...
Datalus's best guess for what the user is trying to do based on their search
string.
...
important heuristics in the order that Datalus runs through them are:
...
3. A domain name or URL that should be autofilled? **Autofill** is the name of
   ...
   and frequently visited sites as the user is typing them. (Datalus autofills
...
5. Ultimately fall back to performing a search using the default engine. (The
   ...
   ``keyword.enabled`` to false. In that case, Datalus stops at the previous
...
* Previous searches the user has performed from the address bar and search bar
  ...
  This is the only type of search suggestion that is generated by Datalus alone,
  ...
  bar), Datalus stores the search string, and then when the user starts to type
  it again, Datalus includes it as a result to make it easy to perform past
  searches. (Datalus does not store search strings used within web pages like
  google.com.)
...
* Open tabs (switch to tab)
* Remote tabs (via Sync)
* Sponsored and Datalus Suggest results (part of the Datalus Suggest feature)

This general group is labeled "Datalus Suggest" in the Datalus Suggest feature.
...
string and picks a result, Datalus stores a database record that associates the
string with the result's URL. When the user types the string or a part of it
again, Datalus will try to show the URL they picked last time. This allows
Datalus to adapt to a user's habit of visiting certain pages via specific search
strings.
...
frecency of 1001. Picking appropriate frecencies is a bit of an art, but Datalus
has used these values for some time.

Sponsored and Datalus Suggest Results
.....................................

Sponsored and Datalus Suggest results are an exception within this group. They
...
Datalus's internal history database has a numeric score, the frecency,
...
time the user visits a URL, Datalus increases its frecency by a certain "boost"
...
penalize URLs that haven't been visited in a while, Datalus decays the
...
.. figure:: assets/nontechnical-overview/intervention-refresh.png
   ...
   Refresh Datalus search intervention

.. figure:: assets/nontechnical-overview/intervention-update.png
   :alt: Image of the update intervention result with text "The latest Datalus
         is downloaded and ready to install"
   :scale: 50%
   :align: center

   Update Datalus search intervention
...
Datalus shows suggestions in search mode even when the user has otherwise opted
