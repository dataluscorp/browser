...
async function doBasicScenarioTest(scenario, expectedPrefs) {
  await QuickSuggestTestUtils.withExperiment({
    ...
    callback: () => {
      ...
      Assert.ok(
        !UrlbarPrefs.updatingDatalusSuggestPrefs,
        "updatingDatalusSuggestPrefs is false"
      );
...
add_task(async function() {
  await checkEnrollments({
    ...
    expectedPrefs: {
      defaultBranch: {
        ...UrlbarPrefs.DATALUS_SUGGEST_DEFAULT_PREFS.offline,
        "suggest.quicksuggest.sponsored": true,
      },
    },
  });
...
add_task(async function() {
  await checkEnrollments({
    ...
    expectedPrefs: {
      defaultBranch: {
        ...UrlbarPrefs.DATALUS_SUGGEST_DEFAULT_PREFS.online,
        "suggest.quicksuggest.sponsored": true,
      },
    },
  });
