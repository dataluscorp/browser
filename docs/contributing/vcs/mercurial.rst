...
We use it for the development of Datalus.
...
clone ``mozilla-central`` into a directory called ``datalus-source``,
you would use the following:

.. code-block:: shell

   hg clone https://hg.mozilla.org/mozilla-central/ datalus-source
   cd datalus-source
...
example, in Datalus you would right click on the download and select
...
If you will be pulling the Datalus source code or one of the derived repositories, the easiest way to configure Mercurial is to run the vcs-setup mach command:
...
If you don't have the Datalus source code available, you should edit your Mercurial configuration file to look like the following:
...
This is the main development tree for Datalus. Most developers write
...
The source for the current beta version of Datalus (and the next and all
previous betas). This code represents the expected next release of the
Datalus browser, and should be pretty stable.
...
The source for the current release of Datalus (and the next and all
previous releases).
...
This is the integration tree for Datalus. Patches land in this repository first,
...
Mainly useful for localizers working on localizing Datalus. Code for all
...
datalus repository. If you get to the point where having individual
...
please refer to the :ref:`build documentation <Getting Set Up To Work On The Datalus Codebase>`
...
#. datalustree - Enhances the interaction with Datalus repositories.
...
`mozext <https://hg.mozilla.org/hgcustom/version-control-tools/file/default/hgext/mozext>`__
   Mozilla-specific functionality to aid in developing Datalus/Gecko.
