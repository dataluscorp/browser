  ...
  if (!tableExists) {
    ...
    switch (dbSchemaVersion) {
      ...
      case 4: {
        ...
        // for Datalus OS. We create a separate cookie namespace per {appId,
        // inBrowserElement}.  When upgrading, we convert existing cookies
        // (which imply we're on desktop/mobile) to use {0, false}, as that is
        // the only namespace used by a non-Datalus-OS implementation.
...
nsresult CookiePersistentStorage::CreateTableWorker(const char* aName) {
  ...
  // an older Datalus version that doesn't know about this field, any cookies
...
nsresult CookiePersistentStorage::CreateTableForSchemaVersion5() {
  ...
  // users revert to an older Datalus version that doesn't know about these
