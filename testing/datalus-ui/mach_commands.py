...
def setup_argument_parser_functional():
    from datalus_ui_harness.arguments.base import DatalusUIArguments
    from mozlog.structured import commandline

    parser = DatalusUIArguments()
    commandline.add_logging_group(parser)
    return parser


def run_datalus_ui_test(testtype=None, topsrcdir=None, **kwargs):
    from mozlog.structured import commandline
    ...
    test_types = {
        "functional": {
            ...
            "cli_module": datalus_ui_harness.cli_functional,
        },
    }

    fxui_dir = os.path.join(topsrcdir, "testing", "datalus-ui")
    ...
    if not kwargs["logger"]:
        kwargs["logger"] = commandline.setup_logging(
            "Datalus UI - {} Tests".format(testtype), {"mach": sys.stdout}
        )
...
@Command(
    ...
    description="Run the functional test suite of Datalus UI tests.",
    parser=setup_argument_parser_functional,
)
def run_datalus_ui_functional(command_context, **kwargs):
    try:
        ...
        command_context.log(
            logging.INFO, "datalus-ui-functional", {"help": e.help()}, "{help}"
        )
        return 1

    return run_datalus_ui_test(
