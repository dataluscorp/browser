...
class TestViewGeckoProfile(unittest.TestCase):
    """Tests the opening local profiles in the Datalus Profiler."""

    def setUp(self):
        self.datalus_profiler_url = None
        self.thread = None
        self.response = [None]
    ...
    def test_view_gecko_profile(self):
        ...
        # user behavior.
        def mocked_open_new_tab(datalus_profiler_url):
            self.datalus_profiler_url = datalus_profiler_url
            encoded_file_url = datalus_profiler_url.split("/")[-1]
            ...
            print("datalus_profiler_url %s" % datalus_profiler_url)
        ...
        expected_url = (
            "https://profiler.datalus.com/from-url/"
            "http%3A%2F%2F127.0.0.1%3A{PORT}%2Ffakeprofile.json"
        )
        actual_url = re.sub("%3A\d+%2F", "%3A{PORT}%2F", self.datalus_profiler_url)

        self.assertEqual(
            actual_url,
            expected_url,
            "The URL generated was correct for the Datalus Profiler.",
        )
        self.assertEqual(
            self.response[0],
            "FAKE_PROFILE",
