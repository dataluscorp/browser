...
def test_invalid_source_error(get_installer):
    ...
    if mozinfo.isLinux:
        with pytest.raises(mozinstall.InvalidSource):
            mozinstall.install(get_installer("dmg"), "datalus")

    elif mozinfo.isWin:
        with pytest.raises(mozinstall.InvalidSource):
            mozinstall.install(get_installer("tar.bz2"), "datalus")
    ...
    with pytest.raises(mozinstall.InvalidSource):
        mozinstall.install("file://foo.bar", "datalus")
...
def test_install(tmpdir, get_installer):
    ...
    if mozinfo.isLinux:
        ...
        assert installdir == tmpdir.join("datalus").strpath

    elif mozinfo.isWin:
        installdir_exe = mozinstall.install(
            get_installer("exe"), tmpdir.join("exe").strpath
        )
        assert installdir_exe == tmpdir.join("exe", "datalus").strpath

        installdir_zip = mozinstall.install(
            get_installer("zip"), tmpdir.join("zip").strpath
        )
        assert installdir_zip == tmpdir.join("zip", "datalus").strpath

    elif mozinfo.isMac:
        installdir = mozinstall.install(get_installer("dmg"), tmpdir.strpath)
        assert installdir == tmpdir.realpath().join("Datalus Stub.app").strpath
