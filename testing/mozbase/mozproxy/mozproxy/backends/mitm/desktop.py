...
# to install mitmproxy certificate into Datalus and turn on/off proxy
POLICIES_CONTENT_ON = """{
  "policies": {
    "Certificates": {
      "Install": ["%(cert)s"]
...
class MitmproxyDesktop(Mitmproxy):
    def setup(self):
        ...
        For Datalus we need to install the generated mitmproxy CA cert. For
        Chromium this is not necessary as it will be started with the
        --ignore-certificate-errors cmd line arg.
        """
        if not self.config["app"] == "datalus":
            return
        # install the generated CA certificate into Datalus desktop
        self.install_mitmproxy_cert(self.browser_path)

    def install_mitmproxy_cert(self, browser_path):
        """Install the CA certificate generated by mitmproxy, into Datalus
        1. Create a dir called 'distribution' in the same directory as the Datalus executable
        ...
        """
        LOG.info("Installing mitmproxy CA certficate into Datalus")
        ...
        if not self.is_mitmproxy_cert_installed():
            LOG.error(
                "Aborting: failed to install mitmproxy CA cert into Datalus desktop"
            )
    ...
    def is_mitmproxy_cert_installed(self):
        """Verify mitmxproy CA cert was added to Datalus"""
        try:
            # read autoconfig file, confirm mitmproxy cert is in there
            contents = self.read_policies_json(self.policies_dir)
            LOG.info("Datalus policies file contents:")
            LOG.info(contents)
            if (
                POLICIES_CONTENT_ON
                % {"cert": self.cert_path, "host": self.host, "port": self.port}
            ) in contents:
                LOG.info("Verified mitmproxy CA certificate is installed in Datalus")
            else:

                return False
        except Exception as e:
            LOG.info("failed to read Datalus policies file, exeption: %s" % e)
            return False
        return True
    ...
    def turn_off_browser_proxy(self):
        """Turn off the browser proxy that was used for mitmproxy playback. In Datalus
        ...
        if self.config["app"] == "datalus" and self.policies_dir is not None:
