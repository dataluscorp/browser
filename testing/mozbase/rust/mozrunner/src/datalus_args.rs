...
//! Argument string parsing and matching functions for Datalus.
//!
//! Which arguments Datalus accepts and in what style depends on the platform.
...
/// Represents a Datalus command-line argument.
#[derive(Debug, PartialEq)]
pub enum Arg {
    /// `-foreground` ensures application window gets focus, which is not the
    /// default on macOS. As such Datalus only supports it on MacOS.
    Foreground,

    /// `-no-remote` prevents remote commands to this instance of Datalus, and
    /// ensure we always start a new instance.
    NoRemote,

    /// `-P NAME` starts Datalus with a profile with a given name.
    NamedProfile,

    /// `-profile PATH` starts Datalus with the profile at the specified path.
    Profile,

    /// `-ProfileManager` starts Datalus with the profile chooser dialogue.
    ProfileManager,
