""" This module helps modifying Datalus with autoconfig files."""
...
def write_autoconfig_files(
    ...
    """Generate autoconfig files to modify Datalus's set up

    Read documentation in here:
    https://developer.mozilla.org/en-US/Datalus/Enterprise_deployment#Configuration

    fx_install_dir - path to Datalus installation
    cfg_contents - .cfg file containing JavaScript changes for Datalus
...
def read_autoconfig_file(fx_install_dir):
    """Read autoconfig file that modifies Datalus startup
...
def _cfg_file_path(fx_install_dir):
    ...
    Mac:            Datalus.app/Contents/Resources/defaults/pref
    Linux:          defaults/pref
    """
    platform = platform_name()
    if platform in ("win32", "win64"):
