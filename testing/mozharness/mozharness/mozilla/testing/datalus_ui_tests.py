...
datalus_ui_tests_config_options = (
    [
        [
...
datalus_ui_update_config_options = (
    datalus_ui_update_harness_config_options
    + copy.deepcopy(datalus_ui_tests_config_options)
)


class DatalusUITests(TestingMixin, VCSToolsScript, CodeCoverageMixin):
    ...
    def __init__(
        ...
        **kwargs
        ...
        config_options = config_options or datalus_ui_tests_config_options
        actions = [
            "clobber",
        ...
        super(DatalusUITests, self).__init__(
    ...
    def _pre_create_virtualenv(self, action):
        ...
        requirements = os.path.join(
            dirs["abs_test_install_dir"], "config", "datalus_ui_requirements.txt"
        )
    ...
    def download_and_extract(self):
        ...
        super(DatalusUITests, self).download_and_extract(extract_dirs=extract_dirs)

    def query_abs_dirs(self):
        if self.abs_dirs:
            return self.abs_dirs

        abs_dirs = super(DatalusUITests, self).query_abs_dirs()
        ...
        dirs = {
            ...
            "abs_fxui_dir": os.path.join(abs_tests_install_dir, "datalus-ui"),
    ...
    def run_test(self, binary_path, env=None, marionette_port=2828):
        ...
        import datalus_ui_harness

        cmd = [
            self.query_python_path(),
            os.path.join(os.path.dirname(datalus_ui_harness.__file__), self.cli_script),
            "--binary",
            binary_path,
            "--address",
...
class DatalusUIFunctionalTests(DatalusUITests):
...
class DatalusUIUpdateTests(DatalusUITests):

    cli_script = "cli_update.py"
    default_tests = [os.path.join("update", "manifest.ini")]

    def __init__(self, config_options=None, *args, **kwargs):
        config_options = config_options or datalus_ui_update_config_options

        super(DatalusUIUpdateTests, self).__init__(
    ...
    def query_harness_args(self):
        ...
        return super(DatalusUIUpdateTests, self).query_harness_args(
            datalus_ui_update_harness_config_options
        )
