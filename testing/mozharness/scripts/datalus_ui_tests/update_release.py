...
from mozharness.mozilla.testing.datalus_ui_tests import (
    DatalusUIUpdateTests,
    datalus_ui_update_config_options,
)

# Command line arguments for release update tests
datalus_ui_update_release_config_options = [
    [
        ["--build-number"],
        {
            "dest": "build_number",
...
] + copy.deepcopy(datalus_ui_update_config_options)


class ReleaseDatalusUIUpdateTests(DatalusUIUpdateTests):
    def __init__(self):
        all_actions = [
            "clobber",
            "checkout",
        ...
        super(ReleaseDatalusUIUpdateTests, self).__init__(
            all_actions=all_actions,
            default_actions=all_actions,
            config_options=datalus_ui_update_release_config_options,
    ...
    def checkout(self):
        ...
        super(ReleaseDatalusUIUpdateTests, self).checkout()
    ...
    def query_abs_dirs(self):
        ...
        abs_dirs = super(ReleaseDatalusUIUpdateTests, self).query_abs_dirs()
        dirs = {
    ...
    def read_release_update_config(self):
        ...
        there is one white space separting them. We only show the values we care for.
            ...
            from="/datalus/releases/38.0b9/linux-x86_64/%locale%/datalus-38.0b9.tar.bz2"
        ...
        for rel_info in sorted(self.releases, key=lambda release: release["build_id"]):
            ...
            for locale in rel_info["locales"]:
                ...
                if retcode:
                    ...
                    base_cmd = (
                        "python {command} --datalus-ui-branch {branch} "
                        "--release-update-config {config} --tools-tag {tag}".format(
                            command=sys.argv[0],
                            branch=self.datalus_ui_branch,
...
if __name__ == "__main__":
    myScript = ReleaseDatalusUIUpdateTests()
