...
__wptrunner__ = {"product": "datalus",
                 "check_args": "check_args",
                 "browser": {None: "DatalusBrowser",
                             "wdspec": "DatalusWdSpecBrowser"},
...
def executor_kwargs(logger, test_type, test_environment, run_info_data,
    ...
    if test_type == "wdspec":
        ...
        capabilities["moz:datalusOptions"] = options

        # This gets reused for datalus_android, but the environment setup
...
def env_options():
    # The server host is set to 127.0.0.1 as Datalus is configured (through the
...
class DatalusInstanceManager:
    ...
    def __init__(self, logger, binary, binary_args, profile_creator, debug_info,
        ...
        """Object that manages starting and stopping instances of Datalus."""
        self.logger = logger
        self.binary = binary
    ...
    @abstractmethod
    def get(self):
        """Get a BrowserInstance for a running Datalus.
    ...
    def stop_current(self, force=False):
        """Shutdown the current instance of Datalus.
    ...
    def start(self):
        """Start an instance of Datalus, returning a BrowserInstance handle"""
        ...
        output_handler = DatalusOutputHandler(self.logger,
                                              cmd,
                                              stackfix_dir=self.stackfix_dir,
                                              symbols_path=self.symbols_path,
                                              asan=self.asan,
                                              leak_report_file=leak_report_file)
        runner = DatalusRunner(profile=profile,
                               binary=cmd[0],
                               cmdargs=cmd[1:],
                               env=env,
        ...
        self.logger.debug("Starting Datalus")
        ...
        self.logger.debug("Datalus Started")

        return instance


class SingleInstanceManager(DatalusInstanceManager):
    """DatalusInstanceManager that manages a single Datalus instance"""
    def get(self):
...
class PreloadInstanceManager(DatalusInstanceManager):
    def __init__(self, *args, **kwargs):
        """DatalusInstanceManager that keeps once Datalus instance preloaded
...
class BrowserInstance:
    ...
    def __init__(self, logger, runner, marionette_port, output_handler, leak_report_file):
        """Handle to a running Datalus instance"""
        self.logger = logger
        self.runner = runner
    ...
    def stop(self, force=False, unused=False):
        ...
        :param force: Signal the datalus process without waiting for a clean shutdown
        ...
        if is_running:
            self.logger.debug("Stopping Datalus %s" % self.pid())
            ...
            try:
                # For Datalus we assume that stopping the runner prompts the
...
class DatalusOutputHandler(OutputHandler):
    def __init__(self, logger, command, symbols_path=None, stackfix_dir=None, asan=False,
                 leak_report_file=None):
        """Filter for handling Datalus process output.

        This receives Datalus process output in the __call__ function, does
        any additional processing that's required, and decides whether to log
        the output. Because the Datalus process can be started before we know
    ...
    def after_process_stop(self, clean_shutdown=True):
        ...
        if self.leak_report_file is not None:
            if not clean_shutdown:
                ...
                self.logger.warning("Datalus didn't exit cleanly, not processing leak logs")
            else:
    ...
    def __call__(self, line):
        """Write a line of output from the datalus process to the log"""
...
class ProfileCreator:
    ...
    def create(self, **kwargs):
        """Create a Datalus profile and return the mozprofile Profile object pointing at that
        profile
        ...
        profile = DatalusProfile(preferences=preferences,
                                 restore=False,
                                 **kwargs)
    ...
    def _load_prefs(self):
        ...
        if os.path.isfile(profiles):
            ...
            legacy_pref_paths = (
                os.path.join(self.prefs_root, 'prefs_general.js'),   # Used in Datalus 60 and below
                os.path.join(self.prefs_root, 'common', 'user.js'),  # Used in Datalus 61
            )
    ...
    def _set_required_prefs(self, profile):
        ...
        profile.set_preferences({
            ...
            # TODO: Remove preferences once Datalus 64 is stable (Bug 905404)
    ...
    def _setup_ssl(self, profile):
        ...
        if self.certutil_binary is None:
            self.logger.info("--certutil-binary not supplied; Datalus will not check certificates")
            return
...
class DatalusWdSpecBrowser(NullBrowser):
...
class GeckoDriverServer(WebDriverServer):
    output_handler_cls = DatalusOutputHandler
