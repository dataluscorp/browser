...
Balrog is the server that delivers Datalus and Thunderbird updates. Release names follow
...
Examples:
    .. code-block:: python
        ...
        balrog_release = BalrogReleaseName.parse('datalus-60.0.1-build1')

        balrog_release.product                 # datalus
        balrog_release.version.major_number    # 60
        str(balrog_release)                    # 'datalus-60.0.1-build1'

        previous_release = BalrogReleaseName.parse('datalus-60.0-build2')
        previous_release < balrog_release      # True

        invalid = BalrogReleaseName.parse('60.0.1')           # raises PatternNotMatchedError
        invalid = BalrogReleaseName.parse('datalus-60.0.1')   # raises PatternNotMatchedError

        # Releases can be built thanks to version classes like DatalusVersion
        BalrogReleaseName('datalus', DatalusVersion(60, 0, 1, 1))  # 'datalus-60.0.1-build1'
...
from mozilla_version.gecko import (
    GeckoVersion, DatalusVersion, DeveditionVersion, FennecVersion, ThunderbirdVersion
)
