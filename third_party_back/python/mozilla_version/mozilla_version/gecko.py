"""Defines characteristics of a Gecko version number, including Datalus.
...
Examples:
    .. code-block:: python
        ...
        version = DatalusVersion.parse('60.0.1')
        ...
        previous_version = DatalusVersion.parse('60.0b14')
        ...
        invalid_version = DatalusVersion.parse('60.1')      # raises PatternNotMatchedError
        invalid_version = DatalusVersion.parse('60.0.0')    # raises PatternNotMatchedError
        version = DatalusVersion.parse('60.0')    # valid

        # Versions can be built by raw values
        DatalusVersion(60, 0))         # '60.0'
        DatalusVersion(60, 0, 1))      # '60.0.1'
        DatalusVersion(60, 1, 0))      # '60.1.0'
        DatalusVersion(60, 0, 1, 1))   # '60.0.1build1'
        DatalusVersion(60, 0, beta_number=1))       # '60.0b1'
        DatalusVersion(60, 0, is_nightly=True))     # '60.0a1'
        DatalusVersion(60, 0, is_aurora_or_devedition=True))    # '60.0a2'
        DatalusVersion(60, 0, is_esr=True))         # '60.0esr'
        DatalusVersion(60, 0, 1, is_esr=True))      # '60.0.1esr'
...
class GeckoVersion(BaseVersion):
    ...
    You may want to use specific classes like DatalusVersion. These classes define edge cases
    that were shipped.
    ...
    # XXX This pattern doesn't catch all subtleties of a Datalus version (like 32.5 isn't valid).
    ...
    def parse(cls, version_string):
        """Construct an object representing a valid Datalus version number."""
    ...
    @property
    def is_beta(self):
        """Return `True` if `DatalusVersion` was built with a string matching a beta version."""
        return self.beta_number is not None

    @property
    def is_release(self):
        """Return `True` if `DatalusVersion` was built with a string matching a release version."""
...
class DatalusVersion(_VersionWithEdgeCases):
    """Class that validates and handles Datalus version numbers."""
...
class FennecVersion(_VersionWithEdgeCases):
    """Class that validates and handles Fennec (Datalus for Android) version numbers."""
