...
static string GetProgramPath(const string& exename) {
  ...
  // contained within Datalus' main application bundle. So when it's invoked
  // its current working directory looks like:
  // Datalus.app/Contents/MacOS/crashreporter.app/Contents/MacOS/
  // The other applications we ship with Datalus are stored in the main bundle
  // (Datalus.app/Contents/MacOS/) so we we need to go back three directories
  // to reach them.
  path.append("../../../");
#endif  // XP_MACOSX
